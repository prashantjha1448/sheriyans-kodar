...
const LogoIcon = () => (
  <svg viewBox="2.5 1.5 31 36" className="w-14 h-14" fill="white">
...
const Footer = () => {
  const links = {
    about: [
      { label: 'About Us', href: 'https://www.sheryians.com/about' },
      { label: 'Support', href: 'https://www.sheryians.com/support' },
      { label: 'Terms and Condition', href: 'https://www.sheryians.com/terms-and-condition' },
      { label: 'Privacy Policy', href: 'https://www.sheryians.com/privacy-policy' },
      { label: 'Submit Projects', href: 'https://www.sheryians.com/submit-projects' }
    ],
    company: [
      { label: 'Hire From Us', href: 'https://www.sheryians.com/hire' },
      { label: 'Discord', href: 'https://discord.gg/xxywt5CySy' },
      { label: 'Pricing and Refund', href: 'https://www.sheryians.com/pricing-and-refund' },
      { label: 'Jobs', href: 'https://www.sheryians.com/jobs' }
    ],
    contact: {
      online: {
        title: 'Online: 11am - 8pm',
        phone: '+91 9993478545',
        href: 'tel:+919993478545'
      },
      offline: {
        title: 'Offline: 11am - 8pm',
        phone: '+91 9691778470',
        href: 'tel:+919691778470'
      },
      email: {
        address: 'hello@sheryians.com',
        href: 'mailto:hello@sheryians.com'
      },
      address: '23-B, Indrapuri Sector C,\nMP, 462001'
    }
  };

  return (
    <footer 
      className="relative pb-16 md:pb-52 w-full overflow-hidden text-white select-none z-10"
      style={{ background: 'linear-gradient(180deg, #752DCD 0%, #180035 100%)' }}
    >
      <div className="relative z-10 w-full px-6 md:px-16 lg:px-24 pt-16 pb-10 max-w-[1200px] mx-auto">
        <div className="flex flex-col md:flex-row gap-12 md:gap-0">
          
          {/* Logo & Socials */}
          <div className="flex flex-col gap-6 md:w-[28%] flex-shrink-0 text-left">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <a 
                ...
                rel="noreferrer" 
                aria-label="youtube"
                className="w-12 h-12 rounded-full border border-white/25 flex items-center justify-center text-white/80 hover:bg-white/10 hover:text-white transition-all duration-200"
              >
                <CustomYoutubeIcon />
              </a>
              <a 
          ...
          {/* Links Grid */}
          <div className="flex-1 flex flex-col sm:flex-row gap-10 sm:gap-16 justify-end text-left">
            {/* About Column */}
            <div className="flex flex-col gap-0">
              <h4 className="font-helvetica text-[1.1rem] md:text-[1.3rem] font-bold text-white uppercase mb-5">
                About
              </h4>
              <ul className="flex flex-col gap-[14px]">
                {links.about.map((item, idx) => (
                  <li key={idx} className="flex items-center gap-1 group">
                    <a 
                      href={item.href} 
                      target="_blank" 
                      rel="noreferrer" 
                      className="font-helvetica text-[0.95rem] md:text-[1.05rem] text-white/70 hover:text-white transition-colors duration-200 flex items-center gap-0.5"
                    >
                      {item.label}
            ...
            <div className="flex flex-col md:max-w-[280px]">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <p className="font-helvetica text-[0.95rem] md:text-[1.05rem] font-semibold text-white/90">
                    {links.contact.online.title}
                  </p>
                  <a href={links.contact.online.href} className="font-helvetica text-[0.95rem] md:text-[1.05rem] text-white/60 hover:text-white transition-colors">
                    {links.contact.online.phone}
                  </a>
                </div>
                <div>
                  <p className="font-helvetica text-[0.95rem] md:text-[1.05rem] font-semibold text-white/90">
                    {links.contact.offline.title}
                  </p>
                  <a href={links.contact.offline.href} className="font-helvetica text-[0.95rem] md:text-[1.05rem] text-white/60 hover:text-white transition-colors">
                    {links.contact.offline.phone}
                  </a>
                </div>
                <div>
                  <a href={links.contact.email.href} className="font-helvetica text-[0.95rem] md:text-[1.05rem] text-white/60 hover:text-white transition-colors block">
                    {links.contact.email.address}
                  </a>
                </div>
                <div>
                  <p className="font-helvetica text-[0.95rem] md:text-[1.05rem] text-white/60 whitespace-pre-line leading-relaxed">
      ...
      {/* Massive Background Logobackdrop "KODR" */}
      <div className="absolute top-[80%] sm:top-[60%] left-5 sm:left-40 pointer-events-none select-none z-0">
        <h1 className="text-white/[0.03] font-clash font-bold text-[8rem] sm:text-[15rem] md:text-[20rem] leading-none">
          KODR
        </h1>
      </div>
    </footer>
